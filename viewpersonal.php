...
<h1 style="text-align:center;background-color:#719a71; width: 100%;">Personal Information</h1>
<?php
  ...
  //echo "Connected successfully\n";
    ...
    echo"<table align=center><tr><th>Birth ID</th><th>Name</th><th>Gender</th><th>Edit</th><th>Delete</th><th>Print</th></tr>";

    if ($result->num_rows > 0) {
      while ($row = $result->fetch_assoc()) {
        // HERE EVERY TABLE DATA WILL CONTAIN DIFFRENT ID TO PRINT
        echo "<tr><td id='a".$row['birth_id']."'>".$row['birth_id']."</td><td id='b".$row['birth_id']."'>".$row['name']."</td>";
        echo "<td id='c".$row['birth_id']."'>".$row['gender']."</td>";
        ...
        if(isset($_GET[$row['birth_id']])){
          ...
          echo "</br></br>";
          echo "Gender: <input type='text' name='gender' value=".$row['gender']."></center>";
          echo "</br>";
          ...
          if(isset($_POST['submit'])){
            ...
            $ssql = "UPDATE personal_info SET birth_id='$birth_id', name='$name', gender='$gender'
        ...
        if(isset($_GET['delete'.$row['birth_id']])){
          $delete = "DELETE FROM personal_info WHERE birth_id=".$row['birth_id']."";
        ...
        if(isset($_GET['print'.$row['birth_id']])){
          ...
          var mywindow = window.open('', 'PRINT', 'height=400,width=600');
					...
					mywindow.document.write('<h1 align=center>' + 'Personal Information'  + '</h1>');
					...
					mywindow.document.write('<th>' + 'Gender' + '</th>');
					mywindow.document.write('</tr>');
  ...
  $conn->close();
   ...
   <h1 style="text-align:center;">Perosonal Information</h1>
        <?php
$servername = "localhost";
$username = "root";
$password = "";
...
if ($result->num_rows > 0) {
    echo "<table align=center><tr><th>Birth ID</th><th>Name</th><th>Gender</th></tr>";
    // output data of each row
    while($row = $result->fetch_assoc()) {
        echo "<tr><td>".$row["birth_id"]."</td><td>".$row["name"]."</td><td>".$row["gender"]."</td></tr>";
    }
    echo "</table>";
}
else {
